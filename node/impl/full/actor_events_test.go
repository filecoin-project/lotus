...
func TestParseHeightRange(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "fails when from is negative",
			heaviest: 500,
			from:     epochPtr(-1),
			to:       nil,
			maxRange: 2880,
			minOut:   0,
			maxOut:   0,
			errStr:   "range 'from' must be greater than or equal to 0",
		},
		{
			name:     "fails when to is negative",
			heaviest: 5000000,
			from:     epochPtr(0),
			to:       epochPtr(-2),
			maxRange: 2880,
			minOut:   0,
			maxOut:   0,
			errStr:   "range 'to' must be greater than or equal to 0",
		},
		{
