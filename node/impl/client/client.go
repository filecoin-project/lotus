...
func (a *API) ClientFindData(ctx context.Context, root cid.Cid) ([]api.QueryOffer, error) {
	...
	for k, p := range peers {
		...
		if err != nil {
			out[k] = api.QueryOffer{Err: err.Error(), Miner: sharedutils.FromSharedAddress(p.Address), MinerPeerID: p.ID}
		} else {
			out[k] = api.QueryOffer{
				Root:        root,
				Size:        queryResponse.Size,
				MinPrice:    sharedutils.FromSharedTokenAmount(queryResponse.PieceRetrievalPrice()),
				Miner:       sharedutils.FromSharedAddress(p.Address), // TODO: check
				MinerPeerID: p.ID,
			}
		}
	}
...
func (a *API) ClientRetrieve(ctx context.Context, order api.RetrievalOrder, path string) error {
	...
	a.Retrieval.Retrieve(
		...
		order.MinerPeerID,
		sharedutils.ToSharedAddress(order.Client),
		sharedutils.ToSharedAddress(order.Miner))
	select {
	case <-ctx.Done():
