...
var ChainNode = Options(
	...
	ApplyIf(isLiteNode,
		...
		Override(new(full.EthTransactionAPIV1), From(new(api.Gateway))),
...
func ConfigFullNode(c interface{}) Option {
	...
	return Options(
		...
		ApplyIf(isFullNode,
			...
			Override(new(full.EthTipSetResolverV1), eth.NewV1TipSetResolver),
			Override(new(full.EthTipSetResolverV2), eth.NewV2TipSetResolver),
