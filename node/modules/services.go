...
func waitForSync(stmgr *stmgr.StateManager, epochs int, subscribe func()) {
	nearsync := time.Duration(epochs*int(build.NetworkParams().BlockDelaySecs())) * time.Second
...
func RandomSchedule(p RandomBeaconParams, _ dtypes.AfterGenesisSet) (beacon.Schedule, error) {
	...
	for _, dc := range p.DrandConfig {
		bc, err := drand.NewDrandBeacon(gen.Timestamp, build.NetworkParams().BlockDelaySecs(), p.PubSub, dc.Config)
		if err != nil {
