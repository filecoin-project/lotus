...
- feat(config): Lotus daemons now serve Ethereum JSON-RPC (`eth_*`) and Actor events out of the box; `Fevm.EnableEthRPC`, `Events.EnableActorEventsAPI`, and `ChainIndexer.EnableIndexer` all default to `true`. ([filecoin-project/lotus#13587](https://github.com/filecoin-project/lotus/pull/13587))
