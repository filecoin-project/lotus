...
- fix(mpool): local message pushes no longer wait for pubsub publication before returning; messages are queued for serialized background publication, with the existing republish loop still handling retry if immediate publish fails
