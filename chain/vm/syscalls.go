...
type SyscallBuilder func(ctx context.Context, rt *Runtime) runtime3.Syscalls

func Syscalls(verifier ffiwrapper.Verifier) SyscallBuilder {
	return func(ctx context.Context, rt *Runtime) runtime3.Syscalls {
...
func (ss *syscallShim) VerifyConsensusFault(a, b, extra []byte) (*runtime3.ConsensusFault, error) {
	...
	var consensusFault *runtime3.ConsensusFault

	// (a) double-fork mining fault
	if blockA.Height == blockB.Height {
		consensusFault = &runtime3.ConsensusFault{
			Target: blockA.Miner,
			Epoch:  blockB.Height,
			Type:   runtime3.ConsensusFaultDoubleForkMining,
		}
	}
	...
	if types.CidArrsEqual(blockA.Parents, blockB.Parents) && blockA.Height != blockB.Height {
		consensusFault = &runtime3.ConsensusFault{
			Target: blockA.Miner,
			Epoch:  blockB.Height,
			Type:   runtime3.ConsensusFaultTimeOffsetMining,
		}
	}
	...
	if len(extra) > 0 {
		...
		if types.CidArrsEqual(blockA.Parents, blockC.Parents) && blockA.Height == blockC.Height &&
			...
			consensusFault = &runtime3.ConsensusFault{
				Target: blockA.Miner,
				Epoch:  blockB.Height,
				Type:   runtime3.ConsensusFaultParentGrinding,
			}
		}
	}
...
func (ss *syscallShim) VerifyPoSt(proof proof3.WindowPoStVerifyInfo) error {
...
func (ss *syscallShim) VerifySeal(info proof3.SealVerifyInfo) error {
...
func (ss *syscallShim) BatchVerifySeals(inp map[address.Address][]proof3.SealVerifyInfo) (map[address.Address][]bool, error) {
	...
	for addr, seals := range inp {
		...
		for i, s := range seals {
			wg.Add(1)
			go func(ma address.Address, ix int, svi proof3.SealVerifyInfo, res []bool) {
				defer wg.Done()
				sema <- struct{}{}
