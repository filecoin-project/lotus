...
// It executes via the FVM's implicit apply path, which requires only that the sender resolves
// to an ID address: the account-type, nonce and balance checks are skipped, and no inclusion
// cost is charged. This enables eth_call and eth_estimateGas to simulate calls from contract
// addresses, matching Geth's behavior. Unlike ApplyImplicitMessage, the message's GasLimit is
// respected and gas costs are reported.
...
func (vm *dualExecutionFVM) ApplyMessageSkipSenderValidation(ctx context.Context, cmsg types.ChainMsg) (ret *ApplyRet, err error) {
	var wg sync.WaitGroup
	...
	go func() {
		defer wg.Done()
		ret, err = vm.main.ApplyMessageSkipSenderValidation(ctx, cmsg)
	}()
