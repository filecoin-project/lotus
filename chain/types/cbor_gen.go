...
var lengthBufMsgSigningMeta = []byte{130}

func (t *MsgSigningMeta) MarshalCBOR(w io.Writer) error {
	if t == nil {
		_, err := w.Write(cbg.CborNull)
		return err
	}
	...
	if _, err := cw.Write(lengthBufMsgSigningMeta); err != nil {
		return err
	}

	// t.Type (types.MsgType) (string)
	...
	if err := cw.WriteMajorTypeHeader(cbg.MajByteString, uint64(len(t.Extra))); err != nil {
		return err
	}

	if _, err := cw.Write(t.Extra[:]); err != nil {
		return err
	}
	return nil
}

func (t *MsgSigningMeta) UnmarshalCBOR(r io.Reader) (err error) {
	*t = MsgSigningMeta{}
	...
	if _, err := io.ReadFull(cr, t.Extra[:]); err != nil {
		return err
	}
	return nil
}
