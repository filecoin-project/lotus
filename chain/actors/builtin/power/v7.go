...
func (s *state7) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state7); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt7.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin7.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
