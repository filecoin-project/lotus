...
func (s *state3) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state3); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt3.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin3.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
