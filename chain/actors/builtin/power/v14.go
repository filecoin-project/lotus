...
func (s *state14) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state14); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt14.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin14.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
