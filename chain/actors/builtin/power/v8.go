...
func (s *state8) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state8); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt8.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin8.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
