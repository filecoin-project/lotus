...
func (s *state10) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state10); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt10.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin10.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
