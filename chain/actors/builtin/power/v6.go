...
func (s *state6) DiffClaims(other State) (*ClaimChanges, error) {

	if o, ok := other.(*state6); ok {
		diff, err := hamt.Diff(s.store.Context(), s.store, o.store, s.Claims, o.Claims, append(adt6.DefaultHamtOptions, hamt.UseTreeBitWidth(builtin6.DefaultHamtBitwidth))...)
		if err != nil {
			return nil, err
		}
		differ := claimDiffer{
