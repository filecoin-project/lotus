...
func MakeInitialStateTree(ctx context.Context, bs bstore.XBlockstore, template genesis.Template) (*state.StateTree, map[address.Address]address.Address, error) {
...
func createMultisigAccount(ctx context.Context, bs bstore.XBlockstore, cst cbor.IpldStore, state *state.StateTree, ida address.Address, info genesis.Actor, keyIDs map[address.Address]address.Address) error {
...
func MakeGenesisBlock(ctx context.Context, j journal.Journal, bs bstore.XBlockstore, sys vm.SyscallBuilder, template genesis.Template) (*GenesisBootstrap, error) {
	if j == nil {
