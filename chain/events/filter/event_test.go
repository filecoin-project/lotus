...
func TestEventFilterCollectEvents(t *testing.T) {
	...
	events14000 := buildTipSetEvents(t, rng, 14000, []*types.Event{ev1})
...
func buildTipSetEvents(tb testing.TB, rng *pseudo.Rand, h abi.ChainEpoch, events []*types.Event) *TipSetEvents {
	tb.Helper()
	...
	ems := []executedMessage{
		{
			msg: fakeMessage(randomActorAddr(tb, rng), randomActorAddr(tb, rng)),
			rct: fakeReceipt(tb, rng, st, events),
			evs: events,
		},
	}
