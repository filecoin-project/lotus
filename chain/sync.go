...
func (syncer *Syncer) collectHeaders(ctx context.Context, incoming *types.TipSet, known *types.TipSet) ([]*types.TipSet, error) {
	...
	if err := syncer.ChainStore().NoteTipSetVisit(ctx, blockSet[len(blockSet)-1], false); err != nil {
		return nil, err
	}
...
loop:
	for blockSet[len(blockSet)-1].Height() > untilHeight {
		...
		if err := syncer.ChainStore().NoteTipSetVisit(ctx, blks[len(blks)-1], false); err != nil {
			return nil, err
		}
	...
	if err := syncer.ChainStore().NoteTipSetVisit(ctx, base, false); err != nil {
		return nil, err
	}
...
func (syncer *Syncer) syncMessagesAndCheckState(ctx context.Context, headers []*types.TipSet) error {
	...
	if err := syncer.ChainStore().NoteTipSetVisit(ctx, headers[len(headers)-1], false); err != nil {
		return err
	}
	...
	return syncer.iterFullTipsets(ctx, headers, func(ctx context.Context, fts *store.FullTipSet) error {
		...
		if err := syncer.ChainStore().NoteTipSetVisit(ctx, fts.TipSet(), true); err != nil {
			return err
		}
