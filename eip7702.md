...
### 2.5 Configuration & Flags (Lotus)

- Build tag: `eip7702_enabled` enables the 7702 send‑path in `ethtypes` for development/testing.
- Env (dev/test): `LOTUS_ETH_7702_DELEGATOR_ADDR` sets the Delegator actor address when the feature flag is enabled.
- Env (policy): `LOTUS_ETH_7702_DELEGATION_CAP` sets the per‑EOA cap for pending ApplyDelegations in the mempool.
- TOML (policy): `Fevm.Eip7702DelegationCap` persists the per‑EOA cap in `config.toml`; the env var can override it at runtime.
