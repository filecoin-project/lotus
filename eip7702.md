...
- Send‑path scaffolding: feature flag (`eip7702_enabled`), atomic CBOR params encoder.
- Focused tests: RLP round‑trip, guards, CBOR shape, feature‑flag path.

Remaining (Phase‑2 back‑half):
- EVM actor: add delegation map + `ApplyAndCall` (validate → write → bump → execute atomically → refund logic).
- Interpreter: CALL pointer semantics and EXTCODE* behavior for delegated EOAs.
...
- Lotus/Go (done): 0x04 parsing/encoding; `authorizationList` in RPC/receipts; atomic CBOR params; send‑path targets EVM.ApplyAndCall (dev/test flag); intrinsic gas overhead scaffold; tests + observability; docs.
...
- FVM/Rust (plan): EVM actor adds delegation state and `ApplyAndCall`; interpreter CALL pointer semantics and EXTCODE* behavior; NV gating centralized.
...
- Env: no special 7702 env required once EVM actor is wired; feature flag controls send‑path in development.
...
- EVM actor owns delegation state and authority recovery.
...
No migration required. The final shape is EVM‑only and atomic‑only; Delegator has been removed. Type‑0x04 transactions are routed exclusively to EVM.ApplyAndCall. Constants: domain `0x05`; pointer bytecode magic/version `0xef 0x01 0x00`; single NV gate `NV_EIP_7702`.
...
* Cross‑account effects: Setting a delegation on an EOA increments the authority’s nonce, which can invalidate pending transactions from that account[eips.ethereum.org](https://eips.ethereum.org/EIPS/eip-7702#:~:text=Allowing%20EOAs%20to%20behave%20as,pending%20from%20the%20sender%E2%80%99s%20account). Lotus tracks nonces per sender; any additional 7702-specific handling is deferred.
...
* Cross‑account nonce updates: Delegating an EOA increments its nonce, potentially invalidating pending transactions from that authority. How should the mempool respond in the future? Could aggressive handling be exploited for denial‑of‑service attacks?
...
* **Extend or add actor code**:
    ...
    * **EVM‑only (final)**:
        ...
        * ApplyAndCall in the EVM actor validates authorization tuples and persists an internal mapping (authority → delegate) and per‑authority nonces.
        
        * The interpreter checks for delegated EOAs and follows pointer semantics while keeping EXTCODE* introspection on the 23‑byte pointer code.
            
* **Update `Eth7702TxArgs.ToUnsignedFilecoinMessage()`**:
    
    * Set `To` to the EVM actor exposing ApplyAndCall.
...
1. The EVM actor maintains an internal map from EOA to delegate address; no separate actor is used.
...
* `TestEstimateGas7702_Behavioral` confirming that refunds are triggered for empty account delegations (behavioral), without asserting exact amounts until constants finalize.
...
* **Automated testing**:
    ...
    * Develop fuzz tests specifically targeting `ValidateAuthorizations`, gas estimation, and mempool handling. These should randomise signature values, nonces and storage conditions.
