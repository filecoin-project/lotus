...
func waitFaultedAndPastDeadline(ctx context.Context, t *testing.T, client *kit.TestFullNode, maddr address.Address, target abi.SectorNumber, targetDeadline uint64, want uint64, maxWait time.Duration) bool {
	...
	require.NoError(t, err)
	// Compare epochs across proving-period boundaries; the last deadline has no
	// greater index after the next period wraps back to deadline zero.
	periodStart := di.PeriodStart
	// NextNotElapsed can put the recorded deadline in the next proving period
	// when the head has advanced past it but its closing cron has not run yet.
	for periodStart > head.Height() {
		periodStart -= di.WPoStProvingPeriod
	}
	deadlineClose := periodStart + abi.ChainEpoch(targetDeadline+1)*di.WPoStChallengeWindow
	if head.Height() >= deadlineClose {
		deadlineClose += di.WPoStProvingPeriod
	}
	endBy := time.Now().Add(maxWait)
	for {
		head, err = client.ChainHead(ctx)
		require.NoError(t, err)
		pw, err := client.StateMinerPower(ctx, maddr, head.Key())
		require.NoError(t, err)
		if pw.MinerPower.QualityAdjPower.Uint64() == want && head.Height() >= deadlineClose {
			faults, err := client.StateMinerFaults(ctx, maddr, head.Key())
			require.NoError(t, err)
			isFaulted, err := faults.IsSet(uint64(target))
			require.NoError(t, err)
			if isFaulted {
				return true
			}
