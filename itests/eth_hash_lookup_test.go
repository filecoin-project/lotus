...
// TestTransactionHashLookupContractDeployment tests to see if lotus correctly stores a mapping from ethereum
// transaction hash to Filecoin Message Cid for a contract deployment
func TestTransactionHashLookupContractDeployment(t *testing.T) {
	kit.QuietMiningLogs()
...
func TestTransactionHashLookupEthValueSend(t *testing.T) {
	kit.QuietMiningLogs()
	...
	robustAddr, err := client.WalletDefaultAddress(ctx)
	require.NoError(t, err)

	idAddr, err := client.StateLookupID(ctx, robustAddr, types.EmptyTSK)
	require.NoError(t, err)

	toEthAddr, err := ethtypes.EthAddressFromFilecoinAddress(idAddr)
	require.NoError(t, err)
