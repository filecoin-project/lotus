...
// TestMigrationNV29SolsticePowerAndFees verifies partition QAP totals and DailyFee re-derivation on USQ.
func TestMigrationNV29SolsticePowerAndFees(t *testing.T) {
	...
	// USQ re-derives DailyFee to the 10x rate.
	...
	// sum ActivePower().QA across all partitions must equal miner QAP.
	...
	// Each partition's QA must equal the sum of its members' own tiers (no whole-partition multiplier).
	err = dls.ForEach(stor, func(dlIdx uint64, dl *stminer.Deadline) error {
		ps, err := dl.PartitionsArray(stor)
		if err != nil {
			return err
