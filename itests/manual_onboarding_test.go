...
func TestManualSectorOnboarding(t *testing.T) {
	...
	for _, withMockProofs := range []bool{true, false} {
		...
		t.Run(testName, func(t *testing.T) {
			...
			require.NoError(t, minerB.WaitTillActivatedAndAssertPower(bSectors, uint64(defaultSectorSize), uint64(defaultSectorSize)*10))
			require.NoError(t, minerC.WaitTillActivatedAndAssertPower(cSectors, uint64(defaultSectorSize), uint64(defaultSectorSize)*10))
