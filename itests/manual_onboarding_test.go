...
func TestManualSectorOnboarding(t *testing.T) {
	...
	for _, withMockProofs := range []bool{true, false} {
		...
		t.Run(testName, func(t *testing.T) {
			...
			bSectors := minerB.OnboardSectors(sealProofType, []kit.SectorManifest{{}})
			req.Len(bSectors, 1)
			...
			cSectors := minerC.OnboardSectors(sealProofType, []kit.SectorManifest{kit.SectorWithRandPiece()})
