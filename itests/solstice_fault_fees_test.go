...
// TestMigrationNV29SolsticeFaultFees compares 1x/10x fault debt, repays the native
// sector's debt, and checks its inactive-sector and recovery-declaration behavior.
func TestMigrationNV29SolsticeFaultFees(t *testing.T) {
	req := require.New(t)
	kit.QuietMiningLogs()
	...
	ledger := func(maddr address.Address) (balance, available, feeDebt abi.TokenAmount) {
	...
	sendFromOwner := func(maddr address.Address, value abi.TokenAmount, method abi.MethodNum, params []byte) {
		...
		req.True(lookup.Receipt.ExitCode.IsSuccess(),
			"message to %s (method %d) must succeed; exit=%d", maddr, method, lookup.Receipt.ExitCode)
	}
	...
	drainWithdrawBalance := func(maddr address.Address, actType string) {
		...
		req.NoError(perr)
		sendFromOwner(maddr, big.Zero(), builtin.MethodsMiner.WithdrawBalance, params)
		balance, avail, debt := ledger(maddr)
		req.True(debt.IsZero(), "%s draining must not create fee debt; got %s", actType, debt)
		req.True(avail.LessThan(types.NewInt(1e6)),
			"%s available must be drained to ~0 after WithdrawBalance; got %s (balance %s)", actType, avail, balance)
	}
	...
	for {
		_, _, d1 := ledger(um1x.ActorAddr)
		_, _, d10 := ledger(um10x.ActorAddr)
	...
	// Continue the native miner's lifecycle on the same drained, faulted sector.
	// Repay first so the USQ rejection below exercises inactivity rather than debt.
	sendFromOwner(um10x.ActorAddr, types.FromFil(10), builtin.MethodSend, nil)
	sendFromOwner(um10x.ActorAddr, big.Zero(), builtin.MethodsMiner.RepayDebt, nil)

	_, available, debt := ledger(um10x.ActorAddr)
	req.True(debt.IsZero(), "RepayDebt must clear FeeDebt back to 0; remaining=%s", debt)
	req.True(available.GreaterThan(big.Zero()), "the miner must hold positive available balance after the top-up")

	// Keep the sector faulted for another period to exercise payment of continuing fees.
	di, err := client.StateMinerProvingDeadline(ctx, um10x.ActorAddr, types.EmptyTSK)
	req.NoError(err)
	client.WaitTillChain(ctx, kit.HeightAtLeast(di.Open+di.WPoStProvingPeriod+10))
	_, _, debt = ledger(um10x.ActorAddr)
	req.True(debt.IsZero(),
		"after one more proving period FeeDebt must still be 0 (top-up covers the continuing fault fee); remaining=%s", debt)

	faults, err := client.StateMinerFaults(ctx, um10x.ActorAddr, types.EmptyTSK)
	req.NoError(err)
	isFaulted, err := faults.IsSet(uint64(native[0]))
	req.NoError(err)
	req.True(isFaulted, "the native 10x sector must remain faulted after another proving period")

	power, err := client.StateMinerPower(ctx, um10x.ActorAddr, types.EmptyTSK)
	req.NoError(err)
	req.True(power.MinerPower.QualityAdjPower.IsZero(),
		"faulting the only native 10x sector must remove all QAP; got %s", power.MinerPower.QualityAdjPower)

	_, err = um10x.UpgradeSectorQuality(native, nil)
	req.Error(err, "USQ on a faulted native sector must be rejected")
	req.Contains(err.Error(), "not active", "USQ on a faulted native sector must fail with 'sector is not active'")

	um10x.RecoverFaults(native)
	recoveries, err := client.StateMinerRecoveries(ctx, um10x.ActorAddr, types.EmptyTSK)
	req.NoError(err)
	isRecovering, err := recoveries.IsSet(uint64(native[0]))
	req.NoError(err)
	req.True(isRecovering, "DeclareFaultsRecovered on the native 10x sector must be accepted and recorded")
