...
func TestGetAllocationIdFromDealId(t *testing.T) {
	ctx := context.Background()
	...
	require.NoError(t, err)
	require.Equal(t, dealInfo.State.VerifiedClaim, verifregtypes.AllocationId(0)) // Allocation in State should not be set yet, because it's in the allocation map

	allocationId, err := api.StateGetDealAllocation(ctx, dealIds[0], types.EmptyTSK)
	require.NoError(t, err)
	require.Equal(t, *allocationId, verifregtypes.AllocationId(3)) // The kit premines 2 verified deals
}
