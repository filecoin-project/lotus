...
import (
	...
	saproof0 "github.com/filecoin-project/specs-actors/actors/runtime/proof"
...
var sealBenchCmd = &cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		var sealedSectors []saproof0.SectorInfo
		...
		if robench != "" {
			...
			for _, s := range genm.Sectors {
				sealedSectors = append(sealedSectors, saproof0.SectorInfo{
					SealedCID:    s.CommR,
		...
		if !c.Bool("skip-commit2") {
			...
			candidates := make([]saproof0.SectorInfo, len(fcandidates))
			...
			pvi1 := saproof0.WinningPoStVerifyInfo{
			...
			pvi2 := saproof0.WinningPoStVerifyInfo{
				Randomness:        abi.PoStRandomness(challenge[:]),
				Proofs:            proof2,
				ChallengedSectors: candidates,
				Prover:            mid,
			...
			wpvi1 := saproof0.WindowPoStVerifyInfo{
			...
			wpvi2 := saproof0.WindowPoStVerifyInfo{
				Randomness:        challenge[:],
				Proofs:            wproof2,
				ChallengedSectors: sealedSectors,
				Prover:            mid,
...
func runSeals(sb *ffiwrapper.Sealer, sbfs *basicfs.Provider, numSectors int, par ParCfg, mid abi.ActorID, sectorSize abi.SectorSize, ticketPreimage []byte, saveC2inp string, skipc2, skipunseal bool) ([]SealingResult, []saproof0.SectorInfo, error) {
	var pieces []abi.PieceInfo
	sealTimings := make([]SealingResult, numSectors)
	sealedSectors := make([]saproof0.SectorInfo, numSectors)
		...
		go func(worker int) {
			sealerr := func() error {
				...
				for i := abi.SectorNumber(start); i < abi.SectorNumber(end); i++ {
					...
					sealedSectors[ix] = saproof0.SectorInfo{
