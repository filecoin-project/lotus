...
func SaveConfigToLayer(minerRepoPath, layerName string, overwrite bool, header http.Header) (err error) {
	...
	if lo.Contains(titles, "base") {
		// append addresses
		var baseCfg config.LotusProviderConfig
		var baseText string
		...
		for _, addr := range baseCfg.Addresses {
			if lo.Contains(addr.MinerAddresses, lpCfg.Addresses[0].MinerAddresses[0]) {
				goto skipWritingToBase
			}
		}
		// write to base
		{
			baseCfg.Addresses = append(baseCfg.Addresses, lpCfg.Addresses[0])
			cb, err := config.ConfigUpdate(baseCfg, config.DefaultLotusProvider(), config.Commented(true), config.DefaultKeepUncommented(), config.NoEnv())
			if err != nil {
	...
	say(code, `lotus-provider `+dbSettings+` config edit `+layerName)
	say(plain, `To run Lotus Provider: in its own machine or cgroup without other files, use the command:`)
	say(code, `lotus-provider `+dbSettings+` run --layer=`+layerName+`,post`)
	return nil
}
