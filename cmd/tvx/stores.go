...
func NewProxyingStores(ctx context.Context, api api.FullNode) *Stores {
	...
	bs := &proxyingBlockstore{
		ctx:         ctx,
		api:         api,
		XBlockstore: blockstore.XNewBlockstore(ds),
	}
	return NewStores(ctx, ds, bs)
}

// NewStores creates a non-proxying set of Stores.
func NewStores(ctx context.Context, ds ds.Batching, bs blockstore.XBlockstore) *Stores {
	var (
...
func (pb *proxyingBlockstore) Get(cid cid.Cid) (blocks.Block, error) {
	...
	if block, err := pb.XBlockstore.Get(cid); err == nil {
		return block, err
	}
...
func (pb *proxyingBlockstore) Put(block blocks.Block) error {
	...
	pb.lk.Unlock()
	return pb.XBlockstore.Put(block)
}
