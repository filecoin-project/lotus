...
```
          ...
          │      Lotus (Go)          │
          │  - Parses 0x04 tx        │
 eth_sendRaw  │  - Encodes CBOR params   │     Mempool
 Transaction ─┤  - Builds msg → Delegator├─▶ policies (cap + eviction)
          │  - RPC views/receipts    │
          └──────────┬───────────────┘
            ...
            │  - ApplyDelegations  │   State (Persistent):
            │    (verify, write,   │   * Mappings (Authority → Delegate)
            │     bump nonces)     │   * Nonces (Per-Authority)
            │  - LookupDelegate    │   * Storage Roots (Per-Authority)
            │  - Get/PutStorageRoot│
            └──────────┬───────────┘
                       │ (during EVM CALL)
                       ▼
