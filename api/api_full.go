...
type FullNode interface {
	...
	// Calling this API gives the lotus node a lease to sign on behalf of given SP.
	// The lease should be active only on one node. The lease will expire at the leaseExpiration.
	// To continue participating in F3 with the given node, call F3Participate again before
	// the leaseExpiration time.
	// leaseExpiration cannot be further than 15 minutes in the future.
	// It is recommended to call F3Participate every 5min with time set 10 min into the future.
	// The minerID has to be the ID address of the miner.
	F3Participate(ctx context.Context, minerID address.Address, leaseExpiration time.Time) error //perm:sign
