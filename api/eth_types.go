...
func ethMarshalJSONAsHex(b [16]byte) ([]byte, error) {
	if len(b) == 0 {
		return json.Marshal("0x0")
	}
...
func ethUnmarshalJSONAsHex(b []byte) ([16]byte, error) {
	var s string
	if err := json.Unmarshal(b, &s); err != nil {
		return [16]byte{}, err
	}
	...
	if len(decoded) > 32 {
		return [16]byte{}, xerrors.Errorf("cannot parse bytes: incorrect input length")
	}

	var ret [16]byte
	copy(ret[16-len(decoded):], decoded)
	return ret, nil
}

type EthFilterID [16]byte
