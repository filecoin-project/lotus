...
// Worker is a separate process that the miner can use to offload phases of the sealing/unsealing pipeline.
// It can run on the same machine as the miner or on a different machine.
// For more information, see https://docs.filecoin.io/mine/lotus/seal-workers/
type Worker interface {
	storiface.WorkerCalls

	// TaskTypes returns the types of sealing/unsealing tasks that the worker supports.
	TaskTypes(context.Context) (map[sealtasks.TaskType]struct{}, error)

	// Paths returns information about the local storage paths present on the Worker.
	// The storage directories located at these paths are managed by the Worker's local sector store.
	Paths(context.Context) ([]stores.StoragePath, error)

	// Info returns information about the worker's resources that is used by the Miner's scheduler
	// to determine which worker to schedule tasks on.
	Info(context.Context) (storiface.WorkerInfo, error)

	// Session returns a random uuid generated by a worker on startup.
	// It uniquely identifies a run of the worker and can be used to identify worker restarts.
...
// AddWorker registers a worker with the miner's work scheduler.
// We query the worker for it's current sessionID (a uuid generated by the worker on each startup)
// to determine if we already know this current run of the worker and therefore discard it as a duplicate or if it's a new run and we need to add it
// to our list of known workers.
// We then repeatedly poll the worker's sessionID to detect restarts by identifying sessionID mismatches.
// If we detect a worker restart, we will remove the current worker run from the list of known workers.
// Upon restart, a worker will register itself again by calling `AddWorker` after generating a new session ID.
// This polling basically acts as a heartbeat in the system.
