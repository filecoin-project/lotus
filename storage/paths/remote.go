...
func (r *Remote) AcquireSector(ctx context.Context, s storiface.SectorRef, existing storiface.SectorFileType, allocate storiface.SectorFileType, pathType storiface.PathType, op storiface.AcquireMode, opts ...storiface.AcquireOption) (storiface.SectorPaths, storiface.SectorPaths, error) {
	...
	if settings.Into == nil {
		...
		releaseStorage, err := r.local.Reserve(ctx, s, toFetch, fetchIDs, overheadTable, MaxStorageUtilizationPercentage)
		if err != nil {
...
func (r *Remote) Reserve(ctx context.Context, sid storiface.SectorRef, ft storiface.SectorFileType, storageIDs storiface.SectorPaths, overheadTab map[storiface.SectorFileType]int, maxPercentage int) (func(), error) {
